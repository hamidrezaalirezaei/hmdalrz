function MAPE_Target = Eunite_Main_NoTemp_Training(X)
% clear all;
% clc;
%% Load Data
...
% set number of Training & Test Data
TrainingData = size(Eunite.Models.AllDays.Data,1) - 30;
TestData = TrainingData+1: size(Eunite.Models.AllDays.Data,1);...size(Eunite.Models.AllDays.Data,1) - size(TrainingData);

%  Normalize Daily Peak Load 
% PeakLoad = Eunite.Models.AllDays.PeakLoad(1:TrainingData);
MinTrainingLoad = Eunite.MinPeakLoad;
MaxTrainingLoad = Eunite.MaxPeakLoad;
% LoadN = Eunite.Models.AllDays.NormalPeakLoad(1:TrainingData);

% Normalize Average Daily Temprature :
% MinTemp = Eunite.MinTemp;
% MaxTemp = Eunite.MaxTemp;
% TempN = Eunite.Models.AllDays.NormalTemp(1:TrainingData);
%% KFCM Clustering
load clustering_result.mat
% data = [LoadN, TempN];
% % Number of Clusters
% gamma = 100.150;
...
% figure; hold on;
% plot(index1, LoadN(Subset1_Max), 'rs');
% plot(index2, LoadN(Subset2_min), 'bd');
% 
% figure;
% subplot(211)
% plot(U(1,:))
...
% ClassPeakLoad = Eunite.Models.AllDays.PeakLoad(Subset1_Max);
ClassLoad = Eunite.Models.AllDays.NormalPeakLoad(Subset1_Max);

% Normalize Average Daily Temprature :
ClassTemp = Eunite.Models.AllDays.NormalTemp(Subset1_Max);
ClassHDay = Eunite.Models.AllDays.Data(Subset1_Max,13);
ClassDayType = Eunite.Models.AllDays.Data(Subset1_Max,6:12);
...
%%%%%%%         Set Target Data         %%%%%%%%%
next = size(Eunite.Models.AllDays.PeakLoad(TestData),1);
InputPattern = zeros(next, M);
InputPattern(1,:) = (ClassLoad(end-M+1:end)');
...
TargetMatrixNoTemp  = [InputPattern, Eunite.Models.AllDays.Data(TestData:end,6:12), Eunite.Models.AllDays.Data(TestData,13)];
...
%% Error Evaluate
TargetPeakLoad = Eunite.Models.AllDays.PeakLoad(TestData);
