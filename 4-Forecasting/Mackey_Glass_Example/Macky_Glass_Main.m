...
train = 500;
% Number of ahead step to predicting
next = 100;
...
options.solver = 'qp';
options.tolKKT = 1e-3;...options.epsilon;
...
MAPE_Training = svrstruct.error.MAPE;
...
figure;
subplot(1,2,1);
plot(traindata(2:end) , traindata(1:end-1));
axis('tight');
ylabel('\itX_{t-1}', 'fontsize', 16);
...
set(findobj('Type', 'line'), 'Color', 'k' , 'LineWidth', 2.5);
set(findobj('Type', 'axes'), 'LineWidth', 2, 'Box', 'on', 'FontName', 'F_lotus Bold', 'fontsize', 18)
...
legend(strdata, 'xp¼¶A [ÄITº', 'yÄI¶pA [ÄITº');
